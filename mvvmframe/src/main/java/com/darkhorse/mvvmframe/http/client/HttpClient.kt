package com.darkhorse.mvvmframe.http.client
    ...
    fun addCookieJar(context: Context): HttpClient {
        mBuilder.cookieJar(object : CookieJar {
            override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) {
                ...
                editor.putString(url.host().toString(), json)
                editor.apply()
            }

            override fun loadForRequest(url: HttpUrl): List<Cookie> {
                val sp = context.getSharedPreferences("HttpPreferences", 0)
                val json = sp.getString(url.host().toString(), "")
                return if(json == ""){
                    ArrayList()
                }else{
                    Gson().fromJson(json, object : TypeToken<List<Cookie>>() {
                    }.type)
                }
